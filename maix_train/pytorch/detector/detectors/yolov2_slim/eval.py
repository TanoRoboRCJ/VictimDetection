...
class Evaluator():
    ...
    def __init__(self, data_root, device, dataset, classes, set_type='val', display=False, use_horovod=False):
        ...
        self.cache_dir = os.path.join(self.devkit_path, 'cache')
        if os.path.exists(self.cache_dir):
            import shutil
            print("clean dataset cache dir")
            shutil.rmtree(self.cache_dir)
            os.makedirs(self.cache_dir)
        self.cachefile = os.path.join(self.cache_dir, 'annots.pkl')
    ...
    def evaluate(self, net):
        if not self.use_horovod or (self.use_horovod and self.hvd.rank() == 0):
            ...
            for i in range(num_images):
                ...
                x = Variable(torch.from_numpy(im).unsqueeze(0)).to(self.device)
                t0 = time.time()
                # forward
            ...
            self.map, aps = self.evaluate_detections(self.all_boxes)
    ...
    def do_python_eval(self, use_07=True):
        ...
        if self.display:
            ...
            self.map = np.mean(aps)
            print('Mean AP = {:.4f}'.format(np.mean(aps)))
        return self.map, zip(self.labelmap, aps)
    ...
    def voc_eval(self, detpath, classname, ovthresh=0.5, use_07_metric=True):
        # read list of images
        ...
        if not os.path.isfile(self.cachefile):
            ...
            # save
            if self.display:
                print('Saving cached annotations to {:s}'.format(self.cachefile))
