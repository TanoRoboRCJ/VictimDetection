...
#         p.join()
#         return gpu
def select_gpu(memory_require=0, tf_gpu_mem_growth=False):
    gpus = tf.config.list_physical_devices('GPU')
    if not gpus:
        return None

    for gpu in gpus:
        # Get the GPU memory and check if it meets the memory requirements
        # memory = gpu.memory_limit
        # if memory >= memory_require:
        return gpu

    # If no GPU meets the memory requirements, return None
    return None
